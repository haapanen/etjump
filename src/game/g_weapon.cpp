...
qboolean G_WeaponIsExplosive(meansOfDeath_t mod) {
  switch (mod) {
    ...
    case MOD_EXPLOSIVE:
    case MOD_TELEFRAG: // Gordon: yes this _SHOULD_ be here,
                       // kthxbye
    case MOD_CRUSH:
      return qtrue;
    default:
...
void Weapon_Knife(gentity_t *ent) {
  ...
  if (tr.entityNum >=
      MAX_CLIENTS) // world brush or non-player entity (no blood)
  {
  ...
  G_Damage(traceEnt, ent, ent, vec3_origin, tr.endpos, (damage + rand() % 5), 0,
           mod);
}
...
void Weapon_Medic(gentity_t *ent) {
  ...
  if (level.time - ent->client->ps.classWeaponTime >
      level.medicChargeTime[ent->client->sess.sessionTeam - 1]) {
    ent->client->ps.classWeaponTime =
        level.time - level.medicChargeTime[ent->client->sess.sessionTeam - 1];
  }

  if (ent->client->sess.skill[SK_FIRST_AID] >= 2) {
    ent->client->ps.classWeaponTime +=
        level.medicChargeTime[ent->client->sess.sessionTeam - 1] * 0.15;
  } else {
    ent->client->ps.classWeaponTime +=
        level.medicChargeTime[ent->client->sess.sessionTeam - 1] * 0.25;
  }
  ...
  trap_EngineerTrace(&tr, viewpos, mins, maxs, tosspos, ent->s.number,
                     MASK_MISSILESHOT);
  if (tr.startsolid) {
    // Arnout: this code is a bit more solid than the previous
    // code
...
/*void Weapon_SatchelCharge(gentity_t *ent) {
    ...
    trap_Trace (&tr, ent->client->ps.origin, mins, maxs, org, ent->s.number,
MASK_SOLID); VectorCopy( tr.endpos, org );
...
void Weapon_MagicAmmo(gentity_t *ent) {
  ...
  if (ent->client->sess.skill[SK_SIGNALS] >= 1) {
    ent->client->ps.classWeaponTime +=
        level.lieutenantChargeTime[ent->client->sess.sessionTeam - 1] * 0.15;
  } else {
    ent->client->ps.classWeaponTime +=
        level.lieutenantChargeTime[ent->client->sess.sessionTeam - 1] * 0.25;
  }

  item = BG_FindItem(ent->client->sess.skill[SK_SIGNALS] >= 1 ? "Mega Ammo Pack"
                                                              : "Ammo Pack");
  VectorCopy(ent->client->ps.viewangles, angles);
...
qboolean ReviveEntity(gentity_t *ent, gentity_t *traceEnt) {
  ...
  int weapons[MAX_WEAPONS /
              (sizeof(int) * 8)]; // JPW NERVE 64 bits for weapons held
                                  //	gentity_t	*traceEnt,
  gentity_t *te;
  ...
  memcpy(weapons, traceEnt->client->ps.weapons,
         sizeof(int) * (MAX_WEAPONS / (sizeof(int) * 8)));

  ClientSpawn(traceEnt, qtrue);

  traceEnt->client->ps.stats[STAT_PLAYER_CLASS] =
      traceEnt->client->sess.playerType;
  ...
  memcpy(traceEnt->client->ps.weapons, weapons,
         sizeof(int) * (MAX_WEAPONS / (sizeof(int) * 8)));
  ...
  traceEnt->client->ps.weapAnim =
      ((traceEnt->client->ps.weapAnim & ANIM_TOGGLEBIT) ^ ANIM_TOGGLEBIT) |
      PM_IdleAnimForWeapon(traceEnt->client->ps.weapon);
  ...
  trap_Trace(&tr, traceEnt->client->ps.origin, traceEnt->client->ps.mins,
             traceEnt->client->ps.maxs, traceEnt->client->ps.origin,
             traceEnt->s.number, MASK_PLAYERSOLID);
  if (tr.allsolid) {
  ...
  if (g_fastres.integer > 0) {
    BG_AnimScriptEvent(&traceEnt->client->ps,
                       traceEnt->client->pers.character->animModelInfo,
                       ANIM_ET_JUMP, qfalse, qtrue);
  } else {
    // DHM - Nerve :: Play revive animation
    BG_AnimScriptEvent(&traceEnt->client->ps,
                       traceEnt->client->pers.character->animModelInfo,
                       ANIM_ET_REVIVE, qfalse, qtrue);
...
void Weapon_Syringe(gentity_t *ent) {
  ...
  // VectorMA (muzzleTrace, -16, forward, muzzleTrace);	// DHM - Back up
  // the start point in case medic is
  ...
  if (tr.fraction < 1.0) {
    ...
    if (traceEnt->client != NULL) {
      ...
      if ((traceEnt->client->ps.pm_type == PM_DEAD) &&
        ...
        // Mad Doc - TDF moved all the revive
        ...
        // Arnout: calculate ranks to update
        // numFinalDead arrays. Have to do it
        // manually as addscore has an early out
  ...
  /* Aciz: Syringe doesn't use ammo, so let's not add it back in case of
  misses
  // DHM - Nerve :: If the medicine wasn't used, give back the ammo
  if (!usedSyringe)
  {
          ent->client->ps.ammoclip[BG_FindClipForWeapon(WP_MEDIC_SYRINGE)]
  += 1;
  }
  */
}
...
// Arnout: crude version of G_RadiusDamage to see if the dynamite can damage a
// func_constructible
int EntsThatRadiusCanDamage(vec3_t origin, float radius, int *damagedList) {
  ...
  // bounding box was checking against radius / sqrt(2) if collision is
  // along box plane
  for (i = 0; i < 3; i++) {
...
static void HandleEntsThatBlockConstructible(gentity_t *constructor,
                                             gentity_t *constructible,
                                             qboolean handleBlockingEnts,
  ...
  int constructibleNonSolidBModel =
  ...
  VectorAdd(constructible->r.absmax, constructible->s.origin2,
            constructible->s.origin2);
  ...
  if (constructible->track && constructible->track[0]) {
    ...
    while (1) {
      ...
      // get the bounding box of all entities in the
    ...
    // make our constructible entities solid so we can check
    // against them trap_LinkEntity( constructible );
    ...
    listedEntities =
        trap_EntitiesInBox(constructible->r.absmin, constructible->r.absmax,
  ...
  for (e = 0; e < listedEntities; e++) {
    ...
    // ignore everything but items, players and missiles
    // (grenades too)
    ...
    // the entity is blocked and it is a player, then warn the
    // player
    if (warnBlockingPlayers && check->s.eType == ET_PLAYER) {
      if ((level.time - check->client->lastConstructibleBlockingWarnTime) >=
          MIN_BLOCKINGWARNING_INTERVAL) {
        trap_SendServerCommand(check->s.number, "cp \"Warning, leave the "
                                                "construction area...\" 1");
        // Gordon: store the entity num to warn
        // the bot
        check->client->lastConstructibleBlockingWarnEnt =
            constructible - g_entities;
      ...
      if( constructible->track &&
      constructible->track[0] ) { for( e = 0; e <
      constructibleEntities; e++ ) { check =
      &g_entities[constructibleList[e]];
  ...
  if (handleBlockingEnts) {
    for (e = 0; e < blockingEntities; e++) {
      ...
      if (block->client || block->s.eType == ET_CORPSE) {
        G_Damage(block, constructible, constructor, NULL, NULL, 9999,
                 DAMAGE_NO_PROTECTION, MOD_CRUSH_CONSTRUCTION);
      } else if (block->s.eType == ET_ITEM && block->item->giType == IT_TEAM) {
        // see if it's a critical entity, one
        // that we can't just simply kill
        // (basically flags)
        Team_DroppedFlagThink(block);
      } else {
        // remove the landmine from both
        // teamlists
        if (block->s.eType == ET_MISSILE &&
            block->methodOfDeath == MOD_LANDMINE) {
          mapEntityData_t *mEnt;

          if ((mEnt = G_FindMapEntityData(&mapEntityData[0],
                                          block - g_entities)) != NULL) {
...
static qboolean TryConstructing(gentity_t *ent) {
  ...
  // see if we are in a trigger_objective_info targetting multiple
  // func_constructibles
  if (constructible->s.eType == ET_CONSTRUCTIBLE &&
      ent->client->touchingTOI->chain) {
    ...
    // make sure the other constructible isn't
    // built/underconstruction/something
  ...
  // see if we are in a trigger_objective_info targetting a
  // func_constructible
  if (constructible->s.eType == ET_CONSTRUCTIBLE &&
      constructible->s.teamNum == ent->client->sess.sessionTeam) {

    if (constructible->s.angles2[0] >=
        250) // have to do this so we don't score multiple times
    {
      return (qfalse);
    }
    ...
    // Check if we can construct - updates the classWeaponTime
    // as well
    ...
    if (constructible->s.angles2[0] <= 0) {
      ...
      if (level.time - constructible->lastHintCheckTime <
          CONSTRUCT_POSTDECAY_TIME) {
        return (qtrue); // likely will come back
                        // soon - so override other
                        // plier bits anyway
      }
      // swap brushmodels if staged
      if (constructible->count2) {
        constructible->grenadeFired++;
        constructible->s.modelindex2 =
            constructible->conbmodels[constructible->grenadeFired - 1];
        // trap_SetBrushModel( constructible,
        // va( "*%i",
        // constructible->conbmodels[constructible->grenadeFired-1]
        // ) );
      }
      ...
      if (!constructible->count2) {
        ...
        if (constructible->grenadeFired == constructible->count2) {
          ...
          switch (constructible->grenadeFired) {
            case 1:
              G_Script_ScriptEvent(constructible,
                                   "buil"
                                   "dsta"
                                   "rt",
                                   "stag"
...

      {
        ...
        VectorAdd(constructible->parent->r.absmin,
                  constructible->parent->r.absmax, mid);
      ...
      // Play sound
      /*			if(
         constructible->parent->spawnflags & 8 ) {
                      constructible->parent->s.loopSound
         = G_SoundIndex( va(
         "sound/world/build_stage%i.wav",
         constructible->s.frame ) ); } else {
                      constructible->s.loopSound =
         G_SoundIndex( va(
         "sound/world/build_stage%i.wav",
         constructible->s.frame ) );
                  }*/

      if (ent->client->touchingTOI->chain && ent->client->touchingTOI->count2) {
        // find the constructible indicator and
        // change team
        mapEntityData_t *mEnt;
        ...
        teamList = indicator->s.teamNum == TEAM_AXIS
                       ? &mapEntityData[1]
                       : &mapEntityData[0]; // inversed
        if ((mEnt = G_FindMapEntityData(teamList, indicator - g_entities)) !=
            NULL) {
    ...
    // trap_SendServerCommand( ent-g_entities, "cp \"Job's
    // done!\" 1");

    // eeeh no point in doing this twice
    // HandleEntsThatBlockConstructible( ent, constructible,
    // qtrue, qfalse );
    if (constructible->count2) {
      // backup...
      // int constructibleModelindex =
      // constructible->s.modelindex;
      int constructibleClipmask = constructible->clipmask;
      int constructibleContents = constructible->r.contents;
      int constructibleNonSolidBModel =
          (constructible->s.eFlags & EF_NONSOLID_BMODEL);
    ...
    /*		if( ent->client->touchingTOI->spawnflags & 4 ) {
       // MESSAGE_OVERRIDE gentity_t* pm = G_PopupMessage(
       PM_CONSTRUCTION ); pm->s.effect3Time = G_StringIndex(
       ent->client->touchingTOI->spawnitem ); pm->s.effect2Time
       = TEAM_AXIS; pm->s.density = -1; // -1 = built (custom
       msg) } else { gentity_t* pm = G_PopupMessage(
       PM_CONSTRUCTION ); pm->s.density =
       ent->client->sess.sessionTeam; pm->s.effect2Time = 0; //
       0 = built pm->s.effect3Time =
       ent->client->touchingTOI->s.teamNum;
            }*/

    AddScore(ent,
             constructible->accuracy); // give drop score to guy who built it

    G_AddSkillPoints(ent, SK_EXPLOSIVES_AND_CONSTRUCTION,
                     constructible->constructibleStats.constructxpbonus);
    G_DebugAddSkillPoints(ent, SK_EXPLOSIVES_AND_CONSTRUCTION,
                          constructible->constructibleStats.constructxpbonus,
                          "finishing a construction");

    // unlink the objective info to get rid of the indicator for
    // now Arnout: don't unlink, we still want the location
    // popup. Instead, constructible_indicator_think got changed
    // to free the indicator when the constructible is
    // constructed
    ...
    // ent->client->ps.classWeaponTime = level.time; // Out of
    // "ammo"

    // if not invulnerable and dynamite-able, create a
    // 'destructable' marker for the other team
    if (!(constructible->spawnflags & CONSTRUCTIBLE_INVULNERABLE) &&
        (constructible->constructibleStats.weaponclass >= 1)) {
      if (!constructible->count2 || constructible->grenadeFired == 1) {
        gentity_t *tent = NULL;
        gentity_t *e;
        e = G_Spawn();
        ...
        while ((tent = G_Find(tent, FOFS(target), constructible->targetname)) !=
               NULL) {
        ...
        // Find the trigger_objective_info that
        // targets us (if not set before)
        {
          gentity_t *tent = NULL;
          while ((tent = G_Find(tent, FOFS(target),
        ...
        // find our marker and update it's
        // coordinates
        for (i = 0, check = g_entities; i < level.num_entities; i++, check++) {
          if (check->s.eType != ET_EXPLOSIVE_INDICATOR &&
              check->s.eType != ET_TANK_INDICATOR &&
          ...
          if (check->r.ownerNum == constructible->s.number) {
            ...
            if (constructible->parent->tagParent) {
              ...
              Q_strncpyz(check->tagName, constructible->parent->tagName,
                         MAX_QPATH);
            } else {
              VectorCopy(constructible->r.absmin, check->s.pos.trBase);
              VectorAdd(constructible->r.absmax, check->s.pos.trBase,
                        check->s.pos.trBase);
...
void AutoBuildConstruction(gentity_t *constructible) {
  ...
  if (constructible->count2) {
    ...
    trap_SetBrushModel(
        constructible,
        va("*%i", constructible->conbmodels[constructible->grenadeFired - 1]));
  ...
  // Arnout: don't unlink, we still want the location popup. Instead,
  // constructible_indicator_think got changed to free the indicator
  // when the constructible is constructed
  //			if( constructible->parent )
  //				trap_UnlinkEntity( constructible->parent
  //);
  ...
  // if not invulnerable and dynamite-able, create a 'destructable'
  // marker for the other team
  if (!(constructible->spawnflags & CONSTRUCTIBLE_INVULNERABLE) &&
      (constructible->constructibleStats.weaponclass >= 1)) {
    if (!constructible->count2 || constructible->grenadeFired == 1) {
      gentity_t *tent = NULL;
      ...
      // Find the trigger_objective_info that targets us
      // (if not set before)
      {
        gentity_t *tent = NULL;
        while ((tent = G_Find(tent, FOFS(target), constructible->targetname)) !=
      ...
      e->s.modelindex2 = constructible->parent->s.teamNum == TEAM_AXIS
                             ? TEAM_ALLIES
                             : TEAM_AXIS;
      e->r.ownerNum = constructible->s.number;
      e->think = explosive_indicator_think;
...
void Weapon_Engineer(gentity_t *ent) {
  ...
  trap_EngineerTrace(&tr, muzzleTrace, NULL, NULL, end, ent->s.number,
                     MASK_SHOT | CONTENTS_TRIGGER);
  ...
  if (G_EmplacedGunIsRepairable(traceEnt, ent)) {
    if (g_engineerChargeTime.integer > 150) {
      ...
      if (ent->client->ps.classWeaponTime +
    ...
    if (traceEnt->health >= 255) {
      ...
      if (traceEnt->sound3to2 != ent->client->sess.sessionTeam) {
        AddScore(ent,
                 WOLF_REPAIR_BONUS); // JPW NERVE
                                     // props to the
                                     // E for the
                                     // fixin'
        G_AddSkillPoints(ent, SK_EXPLOSIVES_AND_CONSTRUCTION, 3.f);
        G_DebugAddSkillPoints(ent, SK_EXPLOSIVES_AND_CONSTRUCTION, 3.f,
                              "repairing a MG42");
      }
    ...
    if (traceEnt->methodOfDeath == MOD_LANDMINE) {
      ...
      trap_EngineerTrace(&tr2, traceEnt->s.pos.trBase, NULL, NULL, base,
                         traceEnt->s.number, MASK_SHOT);

      // ydnar: added "surfaceparm landmine"
      // (SURF_LANDMINE) support
      //%	if(!(tr2.surfaceFlags & (SURF_GRASS |
      // SURF_SNOW | SURF_GRAVEL))
      //||
      if (!(tr2.surfaceFlags & SURF_LANDMINE) ||
        ...
        // rain - #202 - give back the correct
        // charge amount
        if (ent->client->sess.skill[SK_EXPLOSIVES_AND_CONSTRUCTION] >= 3) {
          ent->client->ps.classWeaponTime -=
              .33f *
          ...
          ent->client->ps.classWeaponTime -=
              .5f * level.engineerChargeTime[ent->client->sess.sessionTeam - 1];
        }
        ent->client->sess.aWeaponStats[WS_LANDMINE].atts--;
        return;
        // bani
        //  rain - #384 - check landmine team so
        //  that enemy mines can be disarmed
        ...
        if (G_LandmineUnarmed(traceEnt)) {
          // rain - should be impossible
          // now
          //					if (
          // G_LandmineTeam( traceEnt )
          //!=
          //! ent->client->sess.sessionTeam
          //!)
          // return;
        ...
        if (G_LandmineUnarmed(traceEnt)) {
          ...
          traceEnt->r.contents = 0; // (player can walk
                                    // through)
          trap_LinkEntity(traceEnt);

          // Don't allow disarming for
          // sec (so guy that WAS arming
          // doesn't start disarming it!
        ...
        evilbanigoto:
          ...
          if (traceEnt->health >= 250) // have to do this so
                                       // we don't score
                                       // multiple times
          {
            return;
      ...
      if (traceEnt->health >= 250) // have to do this so we don't score
                                   // multiple times
      {
        return;
      }
...
// bani - no tripmine...
      ...
      if (traceEnt->s.teamNum >= 4) {
        ...
        // Opposing team cannot accidentally arm
        // it
...

        {
          ...
          G_TempTraceIgnorePlayersAndBodies();
          numListedEntities =
              EntsThatRadiusCanDamage(org, traceEnt->splashRadius, entityList);
          G_ResetTempTraceIgnoreEnts();
        ...
        for (i = 0; i < num; i++) {
          ...
          if (hit->s.eType == ET_OID_TRIGGER) {
            ...
            // func_explosive
            if (hit->target_ent &&
                Q_stricmp(hit->target_ent->classname, "func_"
                                                      "explosiv"
                                                      "e")) {
              continue;
            }
        ...
        // Don't allow disarming for sec (so guy
        // that WAS arming doesn't start
        // disarming it!
        traceEnt->timestamp = level.time + 1000;
        traceEnt->health = 5;

        // set teamnum so we can check it for
        // drop/defuse exploit
        traceEnt->s.teamNum = ent->client->sess.sessionTeam;
        ...
        // Gordon: moved down here to prevent
        ...
        // this the actual bounding box of
        // dynamite instead of range, also must
        // snap origin to line up properly
        VectorCopy(traceEnt->r.currentOrigin, origin);
        SnapVector(origin);
        VectorAdd(origin, traceEnt->r.mins, mins);
        VectorAdd(origin, traceEnt->r.maxs, maxs);
          ...
          if (hit->s.eType == ET_OID_TRIGGER) {
            ...
            if (hit->spawnflags & AXIS_OBJECTIVE) {
              if (ent->client->sess.sessionTeam ==
                  TEAM_ALLIES) // transfer score info if this is a bomb scoring
                               // objective
              {
            ...
            if (!(hit->spawnflags & 128) &&
              ...
              if (!(hit->spawnflags & OBJECTIVE_DESTROYED)) {
                AddScore(traceEnt->parent,
                         WOLF_DYNAMITE_PLANT); // give drop score to guy who
                                               // dropped it
        ...
        // bani - reordered this check so its
        // AFTER the primary obj check
        //  Arnout - first see if the dynamite
        //  is planted near a constructable
        ...
        if (traceEnt->health >= 248) // have to do this so we don't
                                     // score multiple times
        {
          return;
        }
        dynamiteDropTeam = traceEnt->s.teamNum; // set this here since
                                                // we wack traceent
                                                // later but want
                                                // teamnum for scoring
        ...
        if (traceEnt->health >= 248) {
          ...
          // bani - eh, why was this
          // commented out? it makes
          // sense, and prevents a
          // sploit.
          ...
          for (i = 0; i < num; i++) {
            ...
            if (hit->s.eType == ET_OID_TRIGGER) {
              ...
              if (ent->client->sess.sessionTeam == TEAM_AXIS) {
                ...
                //"cp \"Axis engineer disarmed the Dynamite!\n\""); bani
                defusedObj = qtrue;
              } else // TEAM_ALLIES
              {
                if ((hit->spawnflags & ALLIED_OBJECTIVE) && (!scored)) {
                  ...
                  scored++;
                  hit->spawnflags &=
                      ~OBJECTIVE_DESTROYED; // "re-activate" objective since it
                                            // wasn't destroyed
                }
                if (hit->target_ent) {
                  G_Script_ScriptEvent(hit->target_ent, "defused", "");
                ...
                //								trap_SendServerCommand(-1,
                //"cp \"Allied engineer disarmed the Dynamite!\n\""); bani
                defusedObj = qtrue;
              }
            }
          }
          // bani - prevent multiple
          // messages here
          if (defusedObj) {
            return;
          }

          // bani - reordered this check
          // so its AFTER the primary
          // obj check
          //  Gordon - first see if the
          //  dynamite was planted near
          ...
          //  destroyed
          {
            ...
            numListedEntities = EntsThatRadiusCanDamage(
                org, traceEnt->splashRadius, entityList);
            ...
            for (e = 0; e < numListedEntities; e++) {
              ...
              if (ent->client->sess.sessionTeam == TEAM_AXIS) {
                ...
                //								trap_SendServerCommand(-1,
                //"cp \"Axis engineer disarmed the Dynamite!\" 2");
              } else // TEAM_ALLIES
              {
                ...
                //								trap_SendServerCommand(-1,
                //"cp \"Allied engineer disarmed the Dynamite!\" 2");
              }
...
// JPW NERVE -- launch airstrike as line of bombs mostly-perpendicular to line
// of grenade travel (close air support should *always* drop parallel to
// friendly lines, tho accidents do happen)
...
void G_AddAirstrikeToCounters(gentity_t *ent) {
  int max =
      std::min(6, static_cast<int>(2 * (ceil(G_TeamCount(ent, (weapon_t)-1) *
                                             0.1f * 10 * 0.01f))));
  if (ent->client->sess.sessionTeam == TEAM_AXIS) {
...
qboolean weapon_checkAirStrike(gentity_t *ent) {
  ...
  // FIXME: this is a stupid workaround. Just store the parent team in
  // the enitity itself and use that - no need to look up the parent
  ...
  if (ent->s.teamNum == TEAM_AXIS) {
    if (level.numActiveAirstrikes[0] > 6 ||
      ...
      /*			te = G_TempEntity(
         ent->parent->s.pos.trBase,
         EV_GLOBAL_CLIENT_SOUND ); te->s.eventParm =
         G_SoundIndex( "axis_hq_airstrike_denied" );
    ...
    if (level.numActiveAirstrikes[1] > 6 ||
      ...
      /*			te = G_TempEntity(
         ent->parent->s.pos.trBase,
         EV_GLOBAL_CLIENT_SOUND ); te->s.eventParm =
         G_SoundIndex( "allies_hq_airstrike_denied" );
         te->s.teamNum = ent->parent->s.clientNum;*/
...
void weapon_callAirStrike(gentity_t *ent) {
  ...
  ent->think = G_ExplodeMissile;
  ent->nextthink = level.time + 950 + NUMBOMBS * 100 +
                   crandom() * 50; // 950 offset is for aircraft flyby
...

  {
    ...
    te->s.eventParm =
        G_SoundIndex("sound/weapons/airstrike/airstrike_plane.wav");
    te->r.svFlags |= SVF_BROADCAST;
  }

  trap_Trace(&tr, ent->s.pos.trBase, NULL, NULL, bomboffset, ent->s.number,
             MASK_SHOT);
  if ((tr.fraction < 1.0) &&
      (!(tr.surfaceFlags &
         SURF_NOIMPACT))) // SURF_SKY)) ) { // JPW NERVE changed for
                          // trenchtoast foggie prollem
  {
    ...
    /*		te = G_TempEntity( ent->parent->s.pos.trBase,
       EV_GLOBAL_CLIENT_SOUND
       ); if ( ent->s.teamNum == TEAM_ALLIES ) { te->s.eventParm
       = G_SoundIndex( "allies_hq_airstrike_abort" ); } else {
  ...
  /*	te = G_TempEntity( ent->parent->s.pos.trBase,
     ...
     ent->parent->client->sess.sessionTeam == TEAM_ALLIES ) {
  ...
  for (j = 0; j < ent->count; j++) {
    RotatePointAroundVector(bombaxis, dir, lookaxis,
                            90 + crandom() *
                                     30); // munge the axis line a bit so
                                          // it's not totally perpendicular
    VectorNormalize(bombaxis);

    VectorCopy(bombaxis, pos);
    VectorScale(pos, (float)(-.5f * BOMBSPREAD * NUMBOMBS), pos);
    VectorAdd(ent->s.pos.trBase, pos,
              pos); // first bomb position
    VectorScale(bombaxis, BOMBSPREAD,
                bombaxis); // bomb drop direction offset

    for (i = 0; i < NUMBOMBS; i++) {
      bomb = G_Spawn();
      bomb->nextthink = level.time + i * 100 + crandom() * 50 + 1000 +
                        (j * 2000); // 1000 for aircraft flyby, other
                                    // term for tumble stagger
      ...
      bomb->s.pos.trType = TR_STATIONARY; // was TR_GRAVITY,  might wanna
                                          // go back to this and drop
                                          // from height
      ...
      VectorCopy(bomb->s.pos.trBase,
                 bomboffset); // make sure bombs fall "on top
                              // of" nonuniform scenery
      bomboffset[2] = traceheight;

      VectorCopy(bomboffset, fallaxis);
      fallaxis[2] = bottomtraceheight;

      trap_Trace(&tr, bomboffset, NULL, NULL, fallaxis, ent - g_entities,
                 bomb->clipmask);
      if (tr.fraction != 1.0) {
        ...
        SnapVectorTowards(bomb->s.pos.trBase,
                          temp); // save net bandwidth

        //				G_RailTrail(
        // skypoint,
        // bomb->s.pos.trBase
        //);
        trap_TraceNoEnts(&tr, skypoint, NULL, NULL, bomb->s.pos.trBase, 0,
                         CONTENTS_SOLID);
        if (tr.fraction < 1.f) {
          G_FreeEntity(bomb);
...
// JPW NERVE -- sound effect for spotter round, had to do this as half-second
// bomb warning
    ...
    switch (sfx) {
      case 0:
        G_AddEvent(ent, EV_GENERAL_SOUND,
                   G_SoundIndex("sound/weapons/artillery/"
                                "artillery_fly_1.wav"));
        break;
      case 1:
        G_AddEvent(ent, EV_GENERAL_SOUND,
...
void artillerySpotterThink(gentity_t *ent) {
  ...
  for (i = 0; i < 7; i++) {
    ...
    bomb->classname = "WP"; // WP == White Phosphorous, so we can check for
                            // bounce noise in grenade bounce routine
    ...
    bomb->clipmask = MASK_MISSILESHOT;
    bomb->s.pos.trType = TR_GRAVITY; // was TR_GRAVITY,  might wanna go back to
                                     // this and drop from height
...
void Weapon_Artillery(gentity_t *ent) {
  ...
  // TAT - 10/27/2002 - moved energy check into a func, so I can use
  // same check in bot code
  ...
  for (i = 0; i < count; i++) {
    ...
    bomb->clipmask = MASK_MISSILESHOT;
    bomb->s.pos.trType = TR_STATIONARY; // was TR_GRAVITY,  might wanna go back
                                        // to this and drop from height
    bomb->s.pos.trTime = level.time;    // move a bit on the very first frame
    if (i) // spotter round is always dead on (OK, unrealistic
           // but more fun)
    {
      bomboffset[0] = crandom() * 250;
      bomboffset[1] = crandom() * 250;
    } else {
      bomboffset[0] = crandom() * 50; // was 0; changed per id request to
                                      // prevent spotter round assassinations
    ...
    VectorCopy(bomb->s.pos.trBase,
               bomboffset); // make sure bombs fall "on top of"
                            // nonuniform scenery
    bomboffset[2] = traceheight;
    ...
    trap_Trace(&trace, bomboffset, NULL, NULL, fallaxis, ent->s.number,
               MASK_SHOT);
    if (trace.fraction != 1.0) {
    ...
    bomb2->s.pos.trType = TR_STATIONARY; // was TR_GRAVITY,  might wanna go back
                                         // to this and drop from height
  ...
  if (ent->client->sess.skill[SK_SIGNALS] >= 2) {
    ...
    ent->client->ps.classWeaponTime +=
        0.66f * level.lieutenantChargeTime[ent->client->sess.sessionTeam - 1];
  } else {
    ent->client->ps.classWeaponTime = level.time;
  }
...
// Also - we shouldn't be rendering the grenade anymore after the smoke stops
// and definately not send it to the client xkan, 12/06/2002 - back to the old
// value 2000, now that it looks like smoke disappears more quickly
...
// (SA) modified so it doesn't have trouble with negative locations (quadrant
// problems)
//			(this was causing some problems with bullet marks
// appearing
// since snapping 			too far off the target surface causes
// the
// the distance between the transmitted impact 			point and the
// actual hit surface larger than the mark radius.  (so nothing shows) )
...
// JPW
// mechanism allows different weapon damage for single/multiplayer; we want
// "balanced" weapons in multiplayer but don't want to alter the existing
// single-player damage items that have already been changed
//
// KLUDGE/FIXME: also modded #defines below to become macros that call this fn
// for minimal impact elsewhere
//
int G_GetWeaponDamage(int weapon) {
  switch (weapon) {
    ...
    case WP_SMOKE_MARKER:
      return 140; // just enough to kill somebody
                  // standing on it
    case WP_MAPMORTAR:
...
==============
Bullet_Endpos
    find target end position for bullet trace based on entities weapon and
accuracy
==============
*/
void Bullet_Endpos(gentity_t *ent, float spread, vec3_t *end) {
  ...
  if (BG_IsScopedWeapon(ent->s.weapon)) {
    // aim dir already accounted for sway of scoped weapons in
    // CalcMuzzlePoints()
    dist *= 2;
    randSpread = qfalse;
  }
...
Bullet_Fire_Extended
    ...
    The original Bullet_Fire still passes through here and functions as it
always has.

    uses for this include shooting through entities (windows, doors, other
players, etc.) and reflecting bullets
==============
*/
qboolean Bullet_Fire_Extended(gentity_t *source, gentity_t *attacker,
                              vec3_t start, vec3_t end, float spread,
  ...
  if (traceEnt->takedamage) {
    G_Damage(traceEnt, attacker, attacker, forward, tr.endpos, damage,
...
void Portal_Touch(gentity_t *self, gentity_t *other, trace_t *trace) {
  ...
  if (!other->client) // If this is not a player, then don't teleport it.
  ...
  if (level.portalTeam == 0) {
    if (self->s.eType == ET_PORTAL_BLUE) {
      // Check that the 'other' portal exists and set it
      // as dest
  ...
  other->lastPortalTime =
      level.time + 100; // .5 second cooldown - maybe too much //NOTE -
...
gentity_t *weapon_gpg40_fire(gentity_t *ent, int grenType) {
  ...
  // start with level throwing and adjust based on angle
  vec3_t tosspos;
  ...
  VectorCopy(viewpos,
             orig_viewpos); // bani - to prevent nade-through-teamdoor sploit
  VectorMA(viewpos, 32, forward, viewpos);

  // bani - to prevent nade-through-teamdoor sploit
  trap_Trace(&tr, orig_viewpos, tv(-4.f, -4.f, 0.f), tv(4.f, 4.f, 6.f), viewpos,
  ...
  if (tr.fraction < 1) // oops, bad launch spot ) {
  {
    ...
    trap_Trace(&tr, viewpos, tv(-4.f, -4.f, 0.f), tv(4.f, 4.f, 6.f), tosspos,
               ent->s.number, MASK_MISSILESHOT);
    if (tr.fraction < 1) // oops, bad launch spot
    {
      VectorCopy(tr.endpos, tosspos);
  ...
  // Ridah, return the grenade so we can do some prediction before
  // deciding if we really want to throw it or not
  return m;
}
...
gentity_t *weapon_grenadelauncher_fire(gentity_t *ent, int grenType) {
  ...
  vec3_t viewpos;
  float upangle = 0,
        pitch; //	start with level throwing and adjust based on angle
  vec3_t tosspos;
  ...
  upangle = std::max(upangle, -50.f); //	now clamped to	-50 / 50
                                      //(don't allow firing straight up/down)
  ...
  if (underhand) {
    // move a little bit more away from the player (so underhand
    // tosses don't get caught on nearby lips)
  ...
  if (grenType == WP_DYNAMITE || grenType == WP_SATCHEL) {
    trap_Trace(&tr, viewpos, tv(-12.f, -12.f, 0.f), tv(12.f, 12.f, 20.f),
               tosspos, ent->s.number, MASK_MISSILESHOT);
  } else if (grenType == WP_LANDMINE) {
    trap_Trace(&tr, viewpos, tv(-16.f, -16.f, 0.f), tv(16.f, 16.f, 16.f),
               tosspos, ent->s.number, MASK_MISSILESHOT);
  } else {
    trap_Trace(&tr, viewpos, tv(-4.f, -4.f, 0.f), tv(4.f, 4.f, 6.f), tosspos,
               ent->s.number, MASK_MISSILESHOT);
  ...
  if (grenType == WP_LANDMINE) {
    if (ent->client->sess.sessionTeam ==
        TEAM_AXIS) // store team so we can generate red or blue
                   // smoke
    {
  ...
  if (grenType == WP_SMOKE_MARKER) {
    m->s.teamNum = ent->client->sess.sessionTeam; // store team so we can
                                                  // generate red or blue smoke
    if (ent->client->sess.skill[SK_SIGNALS] >= 3) {
      m->count = 2;
  ...
  //----(SA)	adjust for movement of character.  TODO: Probably
  // comment in
  // later, but only for forward/back not strafing VectorAdd(
  // m->s.pos.trDelta,
  // ent->client->ps.velocity, m->s.pos.trDelta );	// "real"
  // physics
...
void G_BurnMeGood(gentity_t *self, gentity_t *body) {
  ...
  if (body->client && (body->health <= 0 ||
                       body->flameQuota > 0)) // JPW NERVE was > FLAME_THRESHOLD
  {
...
void CalcMuzzlePoint(gentity_t *ent, int weapon, vec3_t forward, vec3_t right,
  ...
  VectorCopy(ent->r.currentOrigin, muzzlePoint);
  muzzlePoint[2] += ent->client->ps.viewheight;
  // Ridah, this puts the start point outside the bounding box, isn't
  // necessary
...
void CalcMuzzlePoints(gentity_t *ent, int weapon) {
  ...
  { // non ai's take into account scoped weapon 'sway' (just another way
    // aimspread is visualized/utilized)
    ...
    if (BG_IsScopedWeapon(weapon)) {
      ...
      viewang[PITCH] +=
          ZOOM_PITCH_AMPLITUDE * sin(phase) * (spreadfrac + pitchMinAmp);

      phase = level.time / 1000.0 * ZOOM_YAW_FREQUENCY * M_PI * 2;
      viewang[YAW] +=
          ZOOM_YAW_AMPLITUDE * sin(phase) * (spreadfrac + yawMinAmp);
    }
  }
  ...
  //----(SA)	modified the muzzle stuff so that weapons that need to
  // fire down
  // a perfect trace 			straight out of the camera (SP5, Mauser
  // right now) can have that accuracy, but 			weapons
  // that need an offset effect (bazooka/grenade/etc.) can still look
  // like 			they came out of the weap.
  CalcMuzzlePointForActivate(ent, forward, right, up, muzzleTrace);
...
void FireWeapon(gentity_t *ent) {
  ...
  if (g_userAim.integer) {
    ...
    aimSpreadScale += 0.15f; // (SA) just adding a temp /maximum/ accuracy for
                             // player (this will be re-visited in greater
                             // detail :)
    if (aimSpreadScale > 1) {
  ...
  if (ent->client->ps.powerups[PW_OPS_DISGUISED] &&
      ent->s.weapon != WP_SMOKE_BOMB && ent->s.weapon != WP_SATCHEL &&
      ent->s.weapon != WP_SATCHEL_DET) {
    if (!(ent->s.weapon == WP_KNIFE || ent->s.weapon == WP_STEN ||
          ent->s.weapon == WP_SILENCER || ent->s.weapon == WP_SILENCED_COLT ||
          ent->s.weapon == WP_AKIMBO_SILENCEDCOLT ||
          ent->s.weapon == WP_AKIMBO_SILENCEDLUGER || ent->s.weapon == WP_K43 ||
          ent->s.weapon == WP_K43_SCOPE || ent->s.weapon == WP_GARAND ||
  ...
  switch (ent->s.weapon) {
    ...
    case WP_LUGER:
      ...
      break;
    case WP_SILENCER:
      Bullet_Fire(ent, SILENCER_SPREAD * aimSpreadScale, SILENCER_DAMAGE,
                  qtrue);
      break;
    case WP_AKIMBO_LUGER:
      Bullet_Fire(ent, AKIMBO_LUGER_SPREAD * aimSpreadScale,
                  AKIMBO_LUGER_DAMAGE, qtrue);
      break;
    case WP_AKIMBO_SILENCEDLUGER:
      Bullet_Fire(ent, AKIMBO_SILENCEDLUGER_SPREAD * aimSpreadScale,
                  AKIMBO_SILENCEDLUGER_DAMAGE, qtrue);
      break;
    case WP_COLT:
      Bullet_Fire(ent, COLT_SPREAD * aimSpreadScale, COLT_DAMAGE, qtrue);
      break;
    case WP_SILENCED_COLT:
      Bullet_Fire(ent, SILENCED_COLT_SPREAD * aimSpreadScale,
                  SILENCED_COLT_DAMAGE, qtrue);
      break;
    case WP_AKIMBO_COLT:
      Bullet_Fire(ent, AKIMBO_COLT_SPREAD * aimSpreadScale, AKIMBO_COLT_DAMAGE,
                  qtrue);
      break;
    case WP_AKIMBO_SILENCEDCOLT:
      Bullet_Fire(ent, AKIMBO_SILENCEDCOLT_SPREAD * aimSpreadScale,
                  AKIMBO_SILENCEDCOLT_DAMAGE, qtrue);
      break;
    case WP_KAR98:
      aimSpreadScale = 1.0f;
    ...
    case WP_CARBINE:
      ...
      break;
    case WP_FG42SCOPE:
      Bullet_Fire(ent, FG42SCOPE_SPREAD * aimSpreadScale, FG42SCOPE_DAMAGE,
                  qfalse);
      break;
    case WP_FG42:
      ...
      break;
    case WP_GARAND_SCOPE:
      Bullet_Fire(ent, GARANDSCOPE_SPREAD * aimSpreadScale, GARANDSCOPE_DAMAGE,
                  qfalse);
      break;
    case WP_GARAND:
      aimSpreadScale = 1.0f;
    ...
    case WP_MOBILE_MG42:
      if (ent->client->ps.pm_flags & PMF_DUCKED ||
        ...
        Bullet_Fire(ent, MOBILE_MG42_SPREAD * aimSpreadScale,
                    MOBILE_MG42_DAMAGE, qfalse);
      }
      break;
    case WP_K43_SCOPE:
      Bullet_Fire(ent, K43SCOPE_SPREAD * aimSpreadScale, K43SCOPE_DAMAGE,
                  qfalse);
      break;
    case WP_K43:
      aimSpreadScale = 1.0;
    ...
    case WP_PANZERFAUST:
      ...
      if (ent->client->sess.skill[SK_HEAVY_WEAPONS] >= 1) {
        ent->client->ps.classWeaponTime +=
            .66f * level.soldierChargeTime[ent->client->sess.sessionTeam - 1];
      } else {
        ent->client->ps.classWeaponTime = level.time;
      }
      ...
      if (ent->client) {
        vec3_t forward;
        AngleVectors(ent->client->ps.viewangles, forward, NULL, NULL);
        VectorMA(ent->client->ps.velocity, -64, forward,
    ...
    case WP_MORTAR_SET:
      ...
      if (ent->client->sess.skill[SK_HEAVY_WEAPONS] >= 1) {
        ...
        ent->client->ps.classWeaponTime +=
            .5f * level.soldierChargeTime[ent->client->sess.sessionTeam - 1];
      }
    ...
    case WP_SMOKE_BOMB:
      if (ent->s.weapon == WP_SMOKE_BOMB || ent->s.weapon == WP_SATCHEL) {
        ...
        if (ent->client->sess
                .skill[SK_MILITARY_INTELLIGENCE_AND_SCOPED_WEAPONS] >= 2) {
          ent->client->ps.classWeaponTime +=
              .66f *
              level.covertopsChargeTime[ent->client->sess.sessionTeam - 1];
        } else {
      ...
      if (ent->s.weapon == WP_LANDMINE) {
        ...
        if (ent->client->sess.skill[SK_EXPLOSIVES_AND_CONSTRUCTION] >= 3) {
          // rain - bug #202 - use 33%,
          // not 66%, when upgraded. do
          // not penalize the happy fun
          // engineer.
          ent->client->ps.classWeaponTime +=
              .33f *
              level.engineerChargeTime[ent->client->sess.sessionTeam - 1];
    ...
    case WP_PORTAL_GUN:
      ...
      // '1' indicates blue portal, '2' indicates red
      // portal
